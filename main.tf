...
resource "azurerm_network_security_rule" "ns_rules" {
  ...
  source_port_range           = "${split( ",", lookup(var.ns_rules[count.index], "source_port_range", "*" ))}"
  destination_port_range      = "${split( ",", lookup(var.ns_rules[count.index], "destination_port_range", "*" ))}"
