...
resource "azurerm_network_security_rule" "ns_rules" {
  ...
  priority                    = "${lookup(var.ns_rules[count.index], "priority")}"
  ...
  source_port_ranges          = "${split( ",", lookup(var.ns_rules[count.index], "source_port_range", "*" ))}"
  destination_port_ranges     = "${split( ",", lookup(var.ns_rules[count.index], "destination_port_range", "*" ))}"
  source_address_prefix       = "${split( ",", lookup(var.ns_rules[count.index], "source_address_prefix", "*"))}"
  destination_address_prefix  = "${split( ",", lookup(var.ns_rules[count.index], "destination_address_prefix", "*"))}"
